...
%     for scenario = [47,52,107,90,158,134,87,82,20,99,27,23,54,157,55,108,111,132,155,94]
    for lhscenario = 1
        ...
        solar_cap = 1;
        batt_cap = 1;
        gen = readmatrix('Baseline_v4/Scenario'+string(lhscenario)+'/gen_'+string(year)+'.csv');
        flow = readmatrix('Baseline_v4/Scenario'+string(lhscenario)+'/flow_'+string(year)+'.csv');
        ifsum = readmatrix('Baseline_v4/Scenario'+string(lhscenario)+'/ifsum_'+string(year)+'.csv');
        charge = readmatrix('Baseline_v4/Scenario'+string(lhscenario)+'/charege_'+string(year)+'.csv');
        disch = readmatrix('Baseline_v4/Scenario'+string(lhscenario)+'/disch_'+string(year)+'.csv');
        wc = readmatrix('Baseline_v4/Scenario'+string(lhscenario)+'/wc_'+string(year)+'.csv');
        sc = readmatrix('Baseline_v4/Scenario'+string(lhscenario)+'/sc_'+string(year)+'.csv');
...
%         close(2)
        ...
        rbus = mpc.bus;
            ...
            Als = sum(ls(Abus,starttime:starttime+nt-1),1);
            for i = B
                Bbus = [Bbus,rbus(obus(:,1)==i,1)];
            end
            
            Bls = sum(ls(Bbus,starttime:starttime+nt-1),1);
            for i = C
                Cbus = [Cbus,rbus(obus(:,1)==i,1)];
                
            end
            
            Cls = sum(ls(Cbus,starttime:starttime+nt-1),1);
            for i = D
                Dbus = [Dbus,rbus(obus(:,1)==i,1)];
            end
            
            Dls = sum(ls(Dbus,starttime:starttime+nt-1),1);
            for i = E
                Ebus = [Ebus,rbus(obus(:,1)==i,1)];
            end
            
            Els = sum(ls(Ebus,starttime:starttime+nt-1),1);
            for i = F
                Fbus = [Fbus,rbus(obus(:,1)==i,1)];
            end
            
            Fls = sum(ls(Fbus,starttime:starttime+nt-1),1);
            for i = G
                Gbus = [Gbus,rbus(obus(:,1)==i,1)];
            end
            
            Gls = sum(ls(Gbus,starttime:starttime+nt-1),1);
            for i = H
                Hbus = [Hbus,rbus(obus(:,1)==i,1)];
            end
            
            Hls = sum(ls(Hbus,starttime:starttime+nt-1),1);
            for i = I
                Ibus = [Ibus,rbus(obus(:,1)==i,1)];
            end
            
            Ils = sum(ls(Ibus,starttime:starttime+nt-1),1);
            for i = J
                Jbus = [Jbus,rbus(obus(:,1)==i,1)];
            end
            
            Jls = sum(ls(Jbus,starttime:starttime+nt-1),1);
            for i = K
                Kbus = [Kbus,rbus(obus(:,1)==i,1)];
            end
            
            Kls = sum(ls(Kbus,starttime:starttime+nt-1),1);
            
            
            
            binrng = starttime:starttime+nt-1;
            figure(3)
            subplot(6,2,1)
...
% save('mpcbusinfo.mat', 'Abus', 'Bbus', 'Cbus', 'Dbus', 'Ebus', 'Fbus', 'Gbus', 'Hbus', 'Ibus', 'Jbus', 'Kbus');