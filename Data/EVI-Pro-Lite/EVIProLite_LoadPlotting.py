...
def run(scenario_path, temp_path="", api_key = "DEMO_KEY",fname = ""):
    print(fname)
...
#Plotting and Save CSVs with data
    for scenario,row in scenario_csv.iterrows():
        if temp_path == "":
            ...
            loadPlotting(final_result,scenario,filename = fname)
            final_result[scenario].to_csv(os.path.join(os.path.curdir,'OutputData',fname+str(scenario)+".csv"))
...
#Only plot the first week of data unless told otherwise
    ...
    if filename == "":
        filename = "scen"+fname+str(scenario)+"_gridLoad"
